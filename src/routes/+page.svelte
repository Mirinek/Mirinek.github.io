<script lang="ts">
	import ActionWindow from '$lib/ActionWindow.svelte';
	import TaskBar from '$lib/TaskBar.svelte';
	import Icon from '$lib/Icon.svelte';
	import SysMenu from '$lib/SysMenu.svelte';
	import { _ } from 'svelte-i18n';
	import {} from '$lib/locales/i18n';

	let content = [
		{
			id: 'certs',
			ico: '📜',
			dskIco:
				'data:image/png;base64,'
		},
		{
			id: 'contact',
			ico: '📧',
			dskIco:
				'data:image/png;base64,'
		},
		{
			id: 'experience',
			ico: '💼',
			dskIco:
				'data:image/png;base64,'
		},
		{
			id: 'projects',
			ico: '📂',
			dskIco:
				'data:image/png;base64,'
		},
		{
			id: 'vm',
			ico: '🖥️',
			dskIco:
				'data:image/png;base64,'
		},
		{
			id: 'readme',
			ico: '📖',
			dskIco:
				'data:image/png;base64,'
		}
	];
</script>

{#each content as item}
	<Icon
		src={item.dskIco}
		alt={$_(`${item.id}.title`)}
		title={$_(`${item.id}.title`)}
		windowId={item.id}
	></Icon>
{/each}
<img class="tint" style="position: absolute; top: {Math.random() *90}%; left: {Math.random() * 90}%; z-index: -1;" width={Math.random() * 100 + 120} src="images/square.webp" alt="">
<img class="tint" style="position: absolute; top: {Math.random() *90}%; left: {Math.random() * 90}%; z-index: -1;" width={Math.random() * 100 + 120} src="images/square.webp" alt="">
<img class="tint" style="position: absolute; top: {Math.random() *90}%; left: {Math.random() * 90}%; z-index: -1;" width={Math.random() * 100 + 120} src="images/square.webp" alt="">


{#each content as item}
	<ActionWindow
		title={item.ico + ' ' + $_(`${item.id}.title`)}
		id={item.id}
		articles={$_(`${item.id}.articles`)}
	></ActionWindow>
{/each}
<SysMenu></SysMenu>
<TaskBar apps={content}></TaskBar>

<style>
  .tint:nth-of-type(1) {
    filter: sepia(1) hue-rotate(0deg) saturate(6);
  }
  .tint:nth-of-type(2) {
    filter: sepia(1) hue-rotate(120deg) saturate(6);
  }
  .tint:nth-of-type(3) {
    filter: sepia(1) hue-rotate(240deg) saturate(6);
  }
</style>
